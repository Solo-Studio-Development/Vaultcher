...
public class Webhook {
    ...
    public static void sendWebhook(@NotNull String description, @NotNull String color, @NotNull String authorName, @NotNull String authorURL, @NotNull String authorIconURL, @NotNull String footerText, @NotNull String footerIconURL, @NotNull String thumbnailURL, @NotNull String title, @NotNull String imageURL) throws IOException, NoSuchFieldException, IllegalAccessException {
        if (isEnabled()) {
            Webhook webhook = new Webhook(ConfigKeys.WEBHOOK_URL.getString());
            webhook.addEmbed(new Webhook.EmbedObject().setDescription(description).setColor((Color) Color.class.getField(color.toUpperCase()).get(null)).setFooter(footerText, footerIconURL).setThumbnail(thumbnailURL).setTitle(title).setAuthor(authorName, authorURL, authorIconURL).setImage(imageURL));
            webhook.execute();
        }
    }
    ...
    private static class JSONObject {
        ...
        public String toString() {
            return "{" + map.entrySet().stream().map(entry -> quote(entry.getKey()) + ": " + stringifyValue(entry.getValue())).collect(Collectors.joining(", ")) + "}";
        }
        ...
        private String arrayToString(Object[] array) {
            return "[" + Arrays.stream(array).map(element -> element instanceof String ? quote((String) element) : String.valueOf(element)).collect(Collectors.joining(", ")) + "]";
        }

        private String listToString(List<?> list) {
            return "[" + list.stream().map(element -> element instanceof String ? quote((String) element) : String.valueOf(element)).collect(Collectors.joining(", ")) + "]";
        }
    }

    public static String replacePlaceholdersCodeCreate(@NotNull String text, VaultcherCreateEvent event) {
        return text.replace("{player}", event.getPlayer().getName()).replace("{vaultcher}", event.getName()).replace("{uses}", String.valueOf(event.getUses())).replace("{command}", event.getCommand());
    }

    public static String replacePlaceholdersCodeDelete(@NotNull String text, VaultcherDeleteEvent event) {
        return text.replace("{player}", event.getPlayer().getName()).replace("{vaultcher}", event.getName());
    }
    ...
    public static String replacePlaceholdersCodeEditUse(@NotNull String text, VaultcherUseEditEvent event) {
        ...
        return text.replace("{old}", String.valueOf(Vaultcher.getDatabase().getUses(code))).replace("{vaultcher}", event.getName()).replace("{new}", String.valueOf(event.getNewUses()));
    }

    public static String replacePlaceholdersCodeEditCommand(@NotNull String text, VaultcherCommandEditEvent event) {
        String code = event.getName();
        return text.replace("{old}", Vaultcher.getDatabase().getCommand(code)).replace("{vaultcher}", event.getName()).replace("{new}", event.getNewCommand());
    }

    public static String replacePlaceholdersCodeEditName(@NotNull String text, VaultcherNameEditEvent event) {
        String code = event.getName();
        return text.replace("{old}", Vaultcher.getDatabase().getName(code)).replace("{vaultcher}", event.getName()).replace("{new}", event.getNewName());
    }
