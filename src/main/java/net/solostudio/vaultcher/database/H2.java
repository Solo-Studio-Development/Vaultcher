...
public class H2 extends AbstractDatabase {
    ...
    public void createTable() {
        ...
        try (Statement statement = getConnection().createStatement()) {
            statement.execute(vaultcherTableQuery);
            statement.execute(vaultcherPlayersTableQuery);
        } catch (SQLException exception) {
            LoggerUtils.error(exception.getMessage());
        }
    }
    ...
    public void redeemVaultcher(@NotNull String name, @NotNull OfflinePlayer player) {
        ...
        String updateOwnersQuery = "UPDATE vaultcher SET OWNERS = TRIM(BOTH ', ' FROM REPLACE(OWNERS, ?, '')) WHERE VAULTCHER = ?";
